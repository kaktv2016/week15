...
<div class="form-container py-3">
    ...
    <div class="d-flex align-items-center justify-content-between mb-4">
        <div>
            <span class="badge bg-primary bg-opacity-10 text-primary rounded-pill px-3 py-2 fw-semibold mb-2">
                ✍️ New Article Form
            </span>
            <h2 class="fw-bold text-dark mb-1">เขียนบทความใหม่</h2>
            <p class="text-muted small mb-0">กรอกข้อมูลบทความเพื่อเผยแพร่ลงในระบบ Admin Dashboard</p>
        </div>
        ...
        <form method="POST" action="{{ route('admin.insert') }}">
            ...
            <div class="mb-4">
                ...
                <textarea class="form-control @error('content') is-invalid @enderror" 
                          ...
                          rows="5" 
                          name="content" 
                          placeholder="กรอกรายละเอียดและเนื้อหาบทความ...">{{ old('content') }}</textarea>
                @error('content')
                    <div class="invalid-feedback fw-semibold mt-1">
                        {{ $message }}
                    </div>
            ...
            <div class="mb-4">
                ...
                <select class="form-select @error('status') is-invalid @enderror" id="status" name="status">
                    <option value="1" {{ old('status', '1') == '1' ? 'selected' : '' }}>🟢 เปิด (เผยแพร่ทันที)</option>
                    <option value="0" {{ old('status') == '0' ? 'selected' : '' }}>⚪ ปิด (ซ่อนบทความ)</option>
                </select>
                @error('status')
                    <div class="invalid-feedback fw-semibold mt-1">
                        {{ $message }}
